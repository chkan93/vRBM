...
def make_dump_name(it, adder, ct):
    return "iter{0}_critical{1},{2}_{3}cd.saif".format(it, ct.id, ct.num, adder)

def tosaif(iname, oname):
    ## todo here
    run(["vcd2saif", "-input", iname, "-output", oname])
    return
...
def runCmd(it, adder, ct):
    ...
    r = file.read()
    file.close()
    tosaif(DUMP_FILE,join(OUTPUT_FOLDER, make_dump_name(it, adder, ct)) )
    os.remove(DUMP_FILE)
...
def generate(critical_setups=[], adder_types=[], iteration_nums=[]):
    ...
    ZIP = "./data/SAIF/ALL_SAIF_{0}.zip".format(now.strftime("%I_%M%p-%B-%d-%Y"))
    print("Zipping all SAIF into " + ZIP)
    run(["zip", "-r", ZIP] + glob.glob("./dumpFolder/*.saif"))
